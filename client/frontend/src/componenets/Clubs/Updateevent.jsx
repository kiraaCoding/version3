  ...
  const handleClick = async (e) => {
    ...
    if (
        ...
        try {
            ...
            formData.append('lieuEv', event.lieuEv); // Append the file directly

            console.log("hey");

            const response = await axios.put(`http://localhost:8800/events/${eventID}`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
  ...
  return (
    ...
    <div className="flex gap-5 justify-between self-center w-full text-base text-black max-w-[1226px] max-md:flex-wrap max-md:max-w-full">
      <div className="flex  text-left flex-col pt-6 pb-20 max-md:max-w-full">
        ...
        <input type="text" placeholder="Enter lieu discreption" className="justify-center items-start p-3.5 mt-3 outline-none rounded-md bg-grey max-md:pr-5 max-md:max-w-full"onChange={handlechange} name="lieuEv">
        </input>
